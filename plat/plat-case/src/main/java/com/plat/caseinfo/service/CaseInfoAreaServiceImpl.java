...
@Service
public class CaseInfoAreaServiceImpl implements CaseInfoAreaService {
	...
	public Object save(CaseInfoArea caseInfoArea) {
		...
		// 增加上报人所属分部，便于统计
		String reportor = caseInfoArea.getReportor();
		if (reportor != null) {
			String subCompanyId = userRepository.getOne(reportor).getSubCompanyId();
			caseInfoArea.setSubCompany(subCompanyId);
		}
		return new BaseResponse<>(200, "success",caseInfoAreaRepository.save(caseInfoArea));
	}

	@Override
	public Object deleteByIds(String[] ids) {
		// TODO Auto-generated method stub
		caseInfoAreaRepository.deleteByIds(ids);
		return new BaseResponse<>(200, "删除成功");
	}

	@Override
	public Object Update(CaseInfoArea target) {
		// TODO Auto-generated method stub
		Optional<CaseInfoArea> source = caseInfoAreaRepository.findById(target.getId());
		if (source.isPresent()) {
			CaseInfoArea caseInfoArea = (CaseInfoArea) BeanProcessUtils.copy(source.get(), target);
			String reportor = caseInfoArea.getReportor();
			if (reportor != null) {
				String subCompanyId = userRepository.getOne(reportor).getSubCompanyId();
				caseInfoArea.setSubCompany(subCompanyId);
			}
			CaseInfoArea result = caseInfoAreaRepository.save(caseInfoArea);
	...
	public Object find(CaseInfoArea caseInfoArea,Page page) {
		...
		// 创建实例
		Example<CaseInfoArea> example = Example.of(caseInfoArea, matcher);
		Long total = caseInfoAreaRepository.count(example);
		// 分页构造
		...
		if (page.getPageNo() != null && page.getPageSize() !=null ) {
			...
			list = caseInfoAreaRepository.findAll(example, pageable).getContent();
			totalPage = (int) (total % page.getPageSize() == 0 ? total / page.getPageSize() : total / page.getPageSize() + 1);
		} else {
			list = caseInfoAreaRepository.findAll(example);
		}
	...
	public Object find2(int status, Page page,String operator ) {
		...
		String countSql = " select t1.status,t1.id,caseNo,concat(casesource1,'-',casesource2) as casesource, \r\n" + 
				...
				" (select id from caseinfo_requestlog_area where caseid=t1.id and status="+status+" ORDER BY createTime desc LIMIT 0,1) as requestLogId1,\r\n" + 
				" (select id from caseinfo_requestlog_area where caseid=t1.id and status="+status+" and operator='"+operator+"' ORDER BY createTime desc LIMIT 0,1) as requestLogId2 \r\n" + 
				" from caseinfo_area t1  \r\n" + 
